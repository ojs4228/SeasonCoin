...
public class RemoveCoin implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if(label.equalsIgnoreCase("코인삭제")){

            if(player.isOp()){

                if(args.length == 0 || args.length == 1){
                    player.sendMessage("/코인삭제 (플레이어) (코인)");
                }
                if (args.length == 2){
                    ...
                    boolean removeCoin = util.removeCoin(targetPlayer, coin);

                    if (removeCoin) {

                        player.sendMessage(targetPlayer.getName() + " 님의 코인에" + coin + "코인을 삭제했습니다");
                        player.sendMessage(targetPlayer.getName() + "님의 총 코인은" + util.getCoin(targetPlayer) + "코인 입니다");

                    }else {
                        player.sendMessage("코인을 뺄 수 없습니다.");
                    }
